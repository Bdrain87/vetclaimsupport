# App Store Listing - Vet Claim Support

> Complete App Store Connect content for submission and updates.

---

## App Store Connect Fields

### App Name (30 characters max)
```
Vet Claim Support
```

### Subtitle (30 characters max)
```
Win Your VA Disability Claim
```

### Promotional Text (170 characters max)
*Can be updated without app review*
```
New: PACT Act presumptive conditions database. Know if your condition qualifies for easier approval. Built by veterans, for veterans.
```

---

## Description (4000 characters max)

```
Get the VA rating you earned.

Every year, thousands of veterans are underrated or denied because they didn't know what the VA was looking for. Claim consultants charge $10,000+ for information that should be free.

Vet Claim Support puts that power in your hands.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

KNOW WHAT THEY'RE LOOKING FOR

See the exact criteria the VA uses to rate your conditions. The same DBQ forms and rating schedules that C&P examiners use—now you'll know exactly what symptoms and limitations determine your rating percentage.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

DOMINATE YOUR C&P EXAM

Your Compensation & Pension exam is the most important 30 minutes of your claim. Generate a personalized Exam Companion document with:
- Key symptoms to describe for each condition
- What examiners look for in their evaluation
- Common mistakes that hurt ratings
- Specific phrases that align with rating criteria

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

DISCOVER SECONDARY CONDITIONS

That knee injury might be connected to your hip pain. Your PTSD could qualify you for sleep apnea. The app identifies potential secondary service connections you may not have considered—conditions that could significantly increase your combined rating.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

CALCULATE YOUR TRUE RATING

VA math isn't simple addition. Use the same formula the VA uses to calculate your combined disability rating, including:
- Bilateral factor for paired extremities
- Accurate rounding rules
- 2026 compensation rates by dependents
- See exactly what your rating means in monthly pay

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

DOCUMENT YOUR WORST DAYS

The symptom journal captures what the VA needs to see—flare-ups, bad days, and how your conditions actually affect your daily life. Log pain levels, sleep quality, mobility limitations, and medication effects.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

ORGANIZE YOUR EVIDENCE

Track medical records, service treatment records, buddy statements, and nexus letters. Know exactly what evidence you have and what's missing before you file.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

BUILT FOR EVERY VETERAN

- Active Duty: Start documenting now for when you separate
- BDD Filers: File before discharge, get paid Day 1
- Recently Separated: Denied or underrated? Now you'll know why
- Long-Time Veterans: It's never too late. Secondary conditions exist.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

YOUR DATA STAYS YOURS

All information is stored locally on your device. We never see your health data, conditions, or personal information. No accounts required. No cloud uploads. Your privacy, protected.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

$4.99. No subscriptions. No hidden fees.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

DISCLAIMER: This app is for educational purposes only. We are not attorneys, medical professionals, or affiliated with the Department of Veterans Affairs. This app does not provide legal or medical advice. Consult qualified professionals for advice specific to your situation.
```

---

## Keywords (100 characters max)

```
VA,disability,claim,veteran,military,rating,C&P,exam,PTSD,benefits,secondary,nexus,DBQ,compensation
```

---

## What's New (Version Updates)

### Version 2.0
```
Version 2.0 - Complete Redesign

- Redesigned interface with cleaner, more intuitive navigation
- Enhanced C&P Exam preparation with condition-specific talking points
- New PACT Act presumptive conditions database
- Improved secondary conditions finder
- Updated 2026 VA compensation rates
- Performance improvements and bug fixes

Thank you for your service. We're committed to helping you get the rating you earned.
```

---

## Screenshot Requirements

### Screenshot Specifications
- iPhone 6.7" (1290 x 2796)
- iPhone 6.5" (1242 x 2688)
- iPhone 5.5" (1242 x 2208)
- iPad Pro 12.9" (2048 x 2732)
- iPad 11" (1668 x 2388)

### Screenshot 1: Hero/Calculator
- **Caption (45 chars max):** `Know your combined VA rating instantly`
- **Visual:** Calculator showing combined rating with monthly compensation
- **Key Elements:** Rating percentage, monthly pay, bilateral factor

### Screenshot 2: Conditions
- **Caption:** `Track every service-connected condition`
- **Visual:** Conditions list with ratings and secondary connections
- **Key Elements:** Condition cards, rating percentages, connection indicators

### Screenshot 3: Exam Prep
- **Caption:** `Dominate your C&P exam`
- **Visual:** Exam Companion document with condition-specific prep
- **Key Elements:** Talking points, examiner tips, common mistakes

### Screenshot 4: Secondary Finder
- **Caption:** `Discover conditions you didn't know about`
- **Visual:** Secondary condition suggestions
- **Key Elements:** Primary condition, related secondaries, connection strength

### Screenshot 5: Symptom Journal
- **Caption:** `Document your worst days`
- **Visual:** Journal entry with symptom tracking
- **Key Elements:** Pain levels, sleep quality, daily impact

### Screenshot 6: Evidence Tracker
- **Caption:** `Organize your evidence package`
- **Visual:** Evidence checklist with completion status
- **Key Elements:** Document categories, completion indicators, missing items

### Screenshot 7: DBQ Reference
- **Caption:** `See exactly what examiners look for`
- **Visual:** DBQ information showing rating criteria
- **Key Elements:** Form sections, rating breakdowns, requirements

### Screenshot 8: Rating Criteria
- **Caption:** `Know what each rating requires`
- **Visual:** PTSD or back condition rating criteria breakdown
- **Key Elements:** Rating levels (0%, 10%, 30%, etc.), specific criteria

---

## App Preview Video (Optional)

### Duration: 15-30 seconds

### Script:
1. **Open app** → Dashboard showing conditions and rating (2s)
2. **Tap into C&P Exam Prep** → Show Exam Companion generating (4s)
3. **Open Calculator** → Enter ratings → Show combined result (4s)
4. **Browse secondary conditions finder** (3s)
5. **End card:** "Get the rating you earned. $4.99" (2s)

---

## App Store Connect Settings

### Category
- **Primary:** Medical
- **Secondary:** Utilities

### Age Rating
- **Rating:** 17+
- Medical/Treatment Information: Yes (frequent/intense)

### URLs
| Field | URL |
|-------|-----|
| Privacy Policy | https://vetclaimsupport.com/privacy |
| Support | https://vetclaimsupport.com/support |
| Marketing | https://vetclaimsupport.com |

---

## Privacy Nutrition Label

### Data Not Collected
- We do not collect any data from this app

### Data Linked to You
- None

### Data Not Linked to You
- Analytics (anonymized crash reports, usage statistics)

---

## Review Response Templates

### Positive Review
```
Thank you for your service and for sharing your experience. We're honored to help veterans navigate the claims process. Best of luck with your claim!
```

### Constructive Feedback
```
Thank you for your feedback. We're constantly working to improve the app for veterans like you. We've noted your suggestion and will consider it for future updates. If you have additional ideas, please reach out to support@vetclaimsupport.com.
```

### Technical Issue
```
We're sorry you experienced this issue. Please contact us at support@vetclaimsupport.com with details about what happened, and we'll work to resolve it quickly. Thank you for your patience.
```

---

## Seasonal Promotional Text Updates

### Veterans Day (November)
```
This Veterans Day, take control of your claim. Updated with 2026 compensation rates and PACT Act conditions. Thank you for your service.
```

### New Year (January)
```
New year, new claim strategy. Start 2026 with the tools to get the rating you earned. Updated compensation rates now available.
```

### PACT Act Anniversary
```
PACT Act expanded benefits for millions. Check if your conditions qualify for presumptive status. No nexus letter needed.
```

---

## Implementation Checklist

### App Store Connect
- [ ] Update App Name (if changing)
- [ ] Update Subtitle
- [ ] Set Promotional Text
- [ ] Update Full Description
- [ ] Update Keywords
- [ ] Prepare What's New text
- [ ] Set Privacy Policy URL
- [ ] Set Support URL
- [ ] Set Marketing URL
- [ ] Configure Age Rating
- [ ] Set up Privacy Nutrition Labels

### Screenshots (per device size)
- [ ] iPhone 6.7" - 8 screenshots
- [ ] iPhone 6.5" - 8 screenshots
- [ ] iPhone 5.5" - 8 screenshots
- [ ] iPad Pro 12.9" - 8 screenshots
- [ ] iPad 11" - 8 screenshots

### App Preview Video
- [ ] Create 15-30 second video
- [ ] Export for each device size
- [ ] Upload to App Store Connect

### Before Submission
- [ ] Verify all URLs work (privacy, support, marketing)
- [ ] Test app on all supported devices
- [ ] Review all text for typos
- [ ] Confirm pricing is set correctly ($4.99)
- [ ] Submit for review

---

## Notes

- Description uses unicode dividers (━) for visual separation
- All prices in USD
- Disclaimer is legally required - do not remove
- Screenshots should show real app content, not mockups
- Video must be captured from actual device/simulator
