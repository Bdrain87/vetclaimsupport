    ...
    <div className="flex flex-col h-[600px] glass-card overflow-hidden">
      <div className="p-6 border-b border-white/10 flex items-center justify-between bg-white/5">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-[#C8A628] rounded-lg text-[#102039]"><Stethoscope size={20} /></div>
          <h3 className="font-bold italic uppercase tracking-tighter">C&P Prep Simulator</h3>
        </div>
        <Sparkles className="text-[#C8A628] animate-pulse" size={18} />
      </div>
        ...
        {messages.map((m, i) => (
          ...
          >
            <div className={`max-w-[80%] p-4 rounded-2xl text-sm leading-relaxed ${
              m.role === 'user' ? 'bg-[#C8A628] text-[#102039] font-bold' : 'bg-white/5 border border-white/10 text-white'
            }`}>
              {m.text}
            </div>
          </motion.div>
        ))}
        {isLoading && <div className="text-[#C8A628] text-xs font-black animate-pulse">EXAMINER IS REVIEWING 38 CFR CRITERIA...</div>}
      </div>
      ...
      <div className="p-6 bg-white/5 border-t border-white/10 relative">
        ...
        <button
          onClick={handleSend}
          className="absolute right-9 top-1/2 -translate-y-1/2 text-[#C8A628] hover:scale-110 transition-transform disabled:opacity-50"
          disabled={isLoading}
        >
          <Send size={20} />
        </button>
