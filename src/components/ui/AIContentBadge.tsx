...
export function AIContentBadge({ className, timestamp, variant = 'banner' }: AIContentBadgeProps) {
  ...
  if (variant === 'inline') {
    return (
      <span
        className={cn(
          'inline-flex items-center gap-1.5 text-[11px] text-gold/80',
          className,
        )}
      >
        ...
        {formattedTime && (
          <span className="text-gold/50 ml-1">· {formattedTime}</span>
        )}
      </span>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-gold/90 font-medium leading-relaxed">
