...
export function AIContentBadge({ className, timestamp, variant = 'banner' }: AIContentBadgeProps) {
  ...
  if (variant === 'inline') {
    return (
      <span
        className={cn(
          'inline-flex items-center gap-1.5 text-[11px] text-warning/80',
          className,
        )}
      >
        ...
        {formattedTime && (
          <span className="text-warning/50 ml-1">· {formattedTime}</span>
        )}
      </span>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {formattedTime && (
          <p className="text-[10px] text-warning/50 mt-0.5">
