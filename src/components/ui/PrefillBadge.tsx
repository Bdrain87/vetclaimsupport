...
export function PrefillBadge({ onClear }: PrefillBadgeProps) {
  return (
    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gold/10 border border-gold/20 text-[10px] text-foreground font-medium">
      <Database className="h-2.5 w-2.5" />
      Pre-filled from your data
      <button
        type="button"
        onClick={(e) => { e.stopPropagation(); onClear(); }}
        className="ml-0.5 hover:text-[#D4A800] transition-colors"
