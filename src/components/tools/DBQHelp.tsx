  ...
  return (
    <div className="p-8 glass-card border-white/5">
      <div className="flex items-center gap-4 mb-8">
        <div className="p-3 bg-[#C5A442]/10 rounded-2xl text-[#C5A442]">
          <BookOpen size={24} />
        </div>
        <div>
      ...
      <div className="space-y-4">
        {DBQ_GUIDE_DATA.map((tip, i) => (
          <motion.div
            ...
            className="p-5 bg-white/5 border border-white/10 rounded-2xl group hover:border-[#C5A442]/30 transition-all"
          >
            <div className="flex justify-between items-start mb-2">
              <span className="text-[10px] font-black text-[#C5A442] uppercase tracking-widest">{tip.section}</span>
              <div className="px-2 py-0.5 rounded bg-white/10 text-[8px] font-bold text-white/40 uppercase">Insider Edge</div>
            </div>
            <h4 className="text-white font-bold mb-3 flex items-center gap-2">
              <ChevronRight size={14} className="text-[#C5A442]" /> {tip.focus}
            </h4>
