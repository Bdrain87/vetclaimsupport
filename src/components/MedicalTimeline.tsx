...
const TimelineEvent = ({ date, title, detail, isLast }: { date: string, title: string, detail: string, isLast?: boolean }) => (
  <div className="flex gap-4">
    <div className="flex flex-col items-center">
      <div className="h-4 w-4 rounded-full bg-[#C5A442] border-4 border-[#000000] z-10" />
      {!isLast && <div className="w-[2px] h-full bg-white/10 -mt-1" />}
    </div>
    <div className="pb-8">
      <p className="text-[10px] font-black text-[#C5A442] uppercase tracking-[0.2em] mb-1">{date}</p>
...
export const MedicalTimeline = () => {
  return (
    <div className="glass-card p-8">
      ...
      <button className="w-full mt-4 flex items-center justify-center gap-2 text-[10px] font-black text-white/50 uppercase hover:text-[#C5A442] transition-colors">
