...
export default function Dashboard() {
  ...
  return (
    <PageContainer className="space-y-4 animate-fade-in pb-4">
      ...
      {/* Section 5: Smart Reminders (conditional) */}
      {reminders.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Section 6: Unified "What to Do Next" */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                ...
                <Link to={step.route} className="text-gold text-xs hover:text-gold-hl shrink-0 mt-0.5">Go</Link>
              </div>
            ))}
          </div>
        )}
