...
const addPDFFooter = (doc: jsPDFType) => {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text('Generated by Vet Claim Support — not VA-accredited (38 C.F.R. Part 14)', pageWidth / 2, pageHeight - 28, { align: 'center' });
  doc.text('This document is for educational and personal preparation purposes only.', pageWidth / 2, pageHeight - 22, { align: 'center' });
  doc.text('It does not constitute legal or medical advice and does not file claims on your behalf.', pageWidth / 2, pageHeight - 16, { align: 'center' });
...
export const exportServiceHistory = async (entries: ServiceEntry[]) => {
  if (entries.length === 0) {
    logger.warn('No service history entries to export');
    return;
  }
...
export const exportMedicalVisits = async (visits: MedicalVisit[]) => {
  if (visits.length === 0) {
    logger.warn('No medical visits to export');
    return;
  }
...
export const exportSymptoms = async (symptoms: SymptomEntry[]) => {
  if (symptoms.length === 0) {
    logger.warn('No symptoms to export');
    return;
  }
...
export const exportDocuments = async (documents: DocumentItem[]) => {
  if (documents.length === 0) {
    logger.warn('No documents to export');
    return;
  }
...
export const exportAllEvidence = async (data: ClaimsData, options?: { returnBlob?: boolean }) => {
  ...
  doc.setFontSize(28);
  doc.setFont('helvetica', 'bold');
  doc.text('VA Evidence Package', pageWidth / 2, 80, { align: 'center' });
  doc.setFontSize(12);
  doc.setTextColor(180, 180, 180);
  doc.setFont('helvetica', 'normal');
  doc.text('Comprehensive Documentation for VA Disability Claims', pageWidth / 2, 95, { align: 'center' });
  ...
  if (data.separationDate) {
    doc.text(`Separation Date: ${data.separationDate}`, pageWidth / 2, 125, { align: 'center' });
  }
  ...
  let coverY = 155;
  const summaryItems = [
    `${counts.conditions} Conditions`, `${counts.symptoms} Symptom Entries`,
    `${counts.medicalVisits} Medical Visits`, `${counts.medications} Medications`,
    `${counts.exposures} Exposures`, `${counts.buddyContacts} Buddy Contacts`,
    `${counts.documents} Documents`,
  ];
  summaryItems.forEach((item) => {
    doc.text(`•  ${item}`, pageWidth / 2 - 30, coverY);
    coverY += 7;
  });

  doc.setTextColor(100, 100, 100);
  doc.setFontSize(8);
  doc.text('Generated by VetClaimSupport — vetclaimsupport.com', pageWidth / 2, 270, { align: 'center' });
  doc.text('This document is for personal use only. Not affiliated with the VA.', pageWidth / 2, 278, { align: 'center' });

  // --- Table of Contents ---
  doc.addPage();
  let yPos = addPDFHeader(doc, { title: 'Table of Contents' });

  const tocSections: { label: string; hasData: boolean }[] = [
    { label: 'Conditions Being Claimed', hasData: counts.conditions > 0 },
    { label: 'Medical Visits', hasData: counts.medicalVisits > 0 },
    { label: 'Symptom Log', hasData: counts.symptoms > 0 },
    { label: 'Medications', hasData: counts.medications > 0 },
    { label: 'Exposures', hasData: counts.exposures > 0 },
    { label: 'Buddy Contacts', hasData: counts.buddyContacts > 0 },
    { label: 'Documents', hasData: counts.documents > 0 },
  ];
  ...
  tocSections.forEach((section) => {
    if (section.hasData) {
      ...
      doc.setTextColor(...colors.secondary);
      doc.setFont('helvetica', 'normal');
      doc.text(`${sectionNum}. ${section.label}`, 25, yPos);
      sectionNum++;
      yPos += 8;
    }
  });
  ...
  if (data.claimConditions && data.claimConditions.length > 0) {
    doc.addPage();
    yPos = addPDFHeader(doc, { title: 'Conditions Being Claimed' });
  ...
  if (data.medicalVisits && data.medicalVisits.length > 0) {
    doc.addPage();
    yPos = addPDFHeader(doc, { title: 'Medical Visits' });
    data.medicalVisits.forEach((visit) => {
      yPos = checkPageBreak(doc, yPos, 20);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...colors.secondary);
      doc.text(`${visit.date} — ${visit.visitType || 'Visit'}`, 25, yPos);
      yPos += 5;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(...colors.muted);
      if (visit.provider) { doc.text(`Provider: ${visit.provider}`, 30, yPos); yPos += 4; }
      if (visit.reason) { doc.text(`Reason: ${visit.reason}`, 30, yPos); yPos += 4; }
      if (visit.diagnosis) { doc.text(`Diagnosis: ${visit.diagnosis}`, 30, yPos); yPos += 4; }
      yPos += 3;
    });
  }
  ...
  if (data.symptoms && data.symptoms.length > 0) {
    doc.addPage();
    yPos = addPDFHeader(doc, { title: 'Symptom Log' });
    const sorted = [...data.symptoms].sort((a, b) => b.date.localeCompare(a.date));
    sorted.forEach((s) => {
      yPos = checkPageBreak(doc, yPos, 15);
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...colors.secondary);
      doc.text(`${s.date} — ${s.symptom || 'Symptom'} (${s.severity}/10)`, 25, yPos);
      yPos += 4;
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...colors.muted);
      if (s.bodyArea) { doc.text(`Area: ${s.bodyArea}`, 30, yPos); yPos += 4; }
      if (s.notes) {
        const lines = doc.splitTextToSize(s.notes, pageWidth - 55);
        doc.text(lines.slice(0, 2), 30, yPos);
  ...
  if (data.medications && data.medications.length > 0) {
    doc.addPage();
    yPos = addPDFHeader(doc, { title: 'Medications' });
    data.medications.forEach((med) => {
      yPos = checkPageBreak(doc, yPos, 15);
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...colors.secondary);
      doc.text(`${med.name}${med.stillTaking ? '' : ' [discontinued]'}`, 25, yPos);
      yPos += 4;
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...colors.muted);
      if (med.prescribedFor) { doc.text(`For: ${med.prescribedFor}`, 30, yPos); yPos += 4; }
      if (med.sideEffects) { doc.text(`Side effects: ${med.sideEffects}`, 30, yPos); yPos += 4; }
      yPos += 2;
    });
  }
  ...
  if (data.exposures && data.exposures.length > 0) {
    doc.addPage();
    yPos = addPDFHeader(doc, { title: 'Exposures' });
    data.exposures.forEach((exp) => {
      yPos = checkPageBreak(doc, yPos, 15);
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...colors.secondary);
      doc.text(`${exp.date} — ${exp.type || 'Exposure'}`, 25, yPos);
      yPos += 4;
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...colors.muted);
      if (exp.location) { doc.text(`Location: ${exp.location}`, 30, yPos); yPos += 4; }
      if (exp.duration) { doc.text(`Duration: ${exp.duration}`, 30, yPos); yPos += 4; }
      yPos += 2;
    });
  }

  // --- Buddy Contacts Section ---
  if (data.buddyContacts && data.buddyContacts.length > 0) {
    doc.addPage();
    yPos = addPDFHeader(doc, { title: 'Buddy Contacts' });
    data.buddyContacts.forEach((contact) => {
      yPos = checkPageBreak(doc, yPos, 15);
      doc.setFontSize(9);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...colors.secondary);
      doc.text(`${contact.name} — ${contact.relationship || ''}`, 25, yPos);
      yPos += 4;
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...colors.muted);
      if (contact.statementStatus) { doc.text(`Status: ${contact.statementStatus}`, 30, yPos); yPos += 4; }
      if (contact.whatTheyWitnessed) { doc.text(`Witnessed: ${contact.whatTheyWitnessed}`, 30, yPos); yPos += 4; }
      yPos += 2;
    });
  }
...
export const exportMigraines = async (migraines: MigraineEntry[], stats?: { totalLast30Days: number; prostratingLast30Days: number; totalAll: number }) => {
  if (migraines.length === 0) {
    logger.warn('No migraine entries to export');
    return;
  }
